...
def train_xgboost(X_train, X_test, y_train, y_test, param_combinations):
...
def save_results(results, param_grid, best_params, outputDir):
    ...
    try:
        output_path = os.path.join(outputDir, "xgboost_results.json")
...
def save_best_model(best_model, outputDir):
    ...
    try:
        model_path = os.path.join(outputDir, "best_xgboost_model.joblib")
...
def main():
    parser = argparse.ArgumentParser(
        description="XGBoost Trainer for Deepfake Detection",
    ...
    parser.add_argument(
        '--output',
        type=str,
        default="results/images/xgboost",
    ...
    try:
        best_model, best_params, results = train_xgboost(X_train, X_test, y_train, y_test, param_combinations)
